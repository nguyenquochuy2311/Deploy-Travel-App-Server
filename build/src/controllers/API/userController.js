    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            helper.checkPermission(req.user.role_id, 'user_add').then(function (rolePerm) {
...
var findAll = function findAll(req, res) {
  helper.checkPermission(req.user.role_id, 'user_get_all').then(function (rolePerm) {
    User.findAll({
      include: [{
        model: Role,
        as: 'role',
...
var findOne = function findOne(req, res) {
  helper.checkPermission(req.user.role_id, 'user_get').then(function (rolePerm) {
    ...
    return _regeneratorRuntime().wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            helper.checkPermission(req.user.role_id, 'user_update').then(function (rolePerm) {
              User.findByPk(req.params.id).then(function (user) {
                if (user) {
                  User.update({
                    ...
                    user_password: req.body.password ? _bcryptjs["default"].hashSync(req.body.password, _bcryptjs["default"].genSaltSync(10), null) : user.user_password,
...
var destroy = function destroy(req, res) {
  helper.checkPermission(req.user.role_id, 'user_delete').then(function (rolePerm) {
    if (!req.params.id) {
      res.status(400).send({
...
var search = function search(req, res) {
  helper.checkPermission(req.user.role_id, 'user_search').then(function (rolePerm) {
    ...
    } else if (req.params.search_by === 'email') {} else if (req.params.search_by === 'phone') {} else if (req.params.search_by === 'fullname') {}
  })["catch"](function (error) {
    res.status(403).send(error);
  });
};
