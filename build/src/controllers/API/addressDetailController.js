...
var AddressDetail = require('../../models').AddressDetail;

var User = require('../../models').User;

var create = function create(req, res) {
  helper.checkPermission(req.user.role_id, 'user_address_add').then(function (rolePerm) {
    if (!req.body.address_id) {
      ...
      Address.findOne({
        ...
        AddressDetail.create({
          ...
          user_id: req.user.id
        }).then(function (addresseDetail) {
          return res.status(201).send(addresseDetail);
...
var findAll = function findAll(req, res) {
  helper.checkPermission(req.user.role_id, 'user_address_get_all').then(function (rolePerm) {
    AddressDetail.findAll({
      ...
      return res.status(200).send(addressDetail);
    })["catch"](function (error) {
      res.status(400).send(error);
    });
  })["catch"](function (error) {
...
var findOne = function findOne(req, res) {
  helper.checkPermission(req.user.role_id, 'user_address_get').then(function (rolePerm) {
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            helper.checkPermission(req.user.role_id, 'user_address_update').then(function (rolePerm) {
              AddressDetail.findByPk(req.params.id).then(function (addressDetail) {
                Address.findByPk(req.body.address_id).then(function (address) {
                  if (address) {
...
var destroy = function destroy(req, res) {
  helper.checkPermission(req.user.role_id, 'user_address_delete').then(function (rolePerm) {
    if (!req.params.id) {
      res.status(400).send({
        msg: 'Please pass Address ID.'
      });
    } else {
      AddressDetail.findByPk(req.params.id).then(function (address) {
        if (address) {
