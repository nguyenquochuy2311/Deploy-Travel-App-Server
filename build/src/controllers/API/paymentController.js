...
var create = function create(req, res) {
  helper.checkPermission(req.user.role_id, 'payment_add').then(function (rolePerm) {
...
var findAll = function findAll(req, res) {
  helper.checkPermission(req.user.role_id, 'payment_get_all').then(function (rolePerm) {
    Payment.findAll({
      include: [{
        model: Booking,
        as: 'booking'
...
var findOne = function findOne(req, res) {
  helper.checkPermission(req.user.role_id, 'payment_get').then(function (rolePerm) {
    Payment.findOne({
      include: [{
        model: Booking,
        as: 'booking'
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            helper.checkPermission(req.user.role_id, 'payment_update').then(function (rolePerm) {
              Payment.findByPk(req.params.id).then(function (payment) {
                Booking.findByPk(req.body.booking_id).then(function (booking) {
                  if (booking) {
                    Payment.update({
...
var destroy = function destroy(req, res) {
  helper.checkPermission(req.user.role_id, 'payment_delete').then(function (rolePerm) {
    if (!req.params.id) {
      res.status(400).send({
