    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            helper.checkPermission(req.user.role_id, 'permissions_add').then(function (rolePerm) {
              if (!req.body.perm_name || !req.body.perm_description) {
                ...
                Permission.create({
                  ...
                  return res.status(201).send(perm);
...
var findAll = function findAll(req, res) {
  helper.checkPermission(req.user.role_id, 'permissions_get_all').then(function (rolePerm) {
    Permission.findAll().then(function (perms) {
...
var findOne = function findOne(req, res) {
  helper.checkPermission(req.user.role_id, 'permissions_get').then(function (rolePerm) {
    Permission.findByPk(req.params.id).then(function (perms) {
      return res.status(200).send(perms);
    })["catch"](function (error) {
      res.status(400).send(error);
    });
...
var update = function update(req, res) {
  helper.checkPermission(req.user.role_id, 'permissions_update').then(function (rolePerm) {
    if (!req.params.id || !req.body.perm_name || !req.body.perm_description) {
      res.status(400).send({
        message: 'Please pass permission ID, name or description.'
      });
    } else {
      Permission.findByPk(req.params.id).then(function (perm) {
        Permission.update({
...
var destroy = function destroy(req, res) {
  helper.checkPermission(req.user.role_id, 'permissions_delete').then(function (rolePerm) {
    if (!req.params.id) {
      res.status(400).send({
